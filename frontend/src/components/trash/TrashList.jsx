        ...
        {loading ? (
          ...
          <div className="flex-grow-1 overflow-auto">
            {trashItems.map((item) => (
                <div
                  key={item.restoreId}
                  className={`video-list-card d-flex gap-4 align-items-start justify-content-between mb-4 ${selectedItems.includes(item.restoreId) ? 'bg-danger bg-opacity-50 p-3' : ''}`}
                  onClick={() => handleSelectItem(item.restoreId)}
                >
                <div className="d-flex gap-4 align-items-start min-w-0 flex-grow-1 position-relative">
